...
from Plugins.Extensions.CamdMenager import *
...
def Plugins(**kwargs):
    return [PluginDescriptor(name='Info Panel', description='Info panel GUI 27/12/2013', where=PluginDescriptor.WHERE_MENU, fnc=Apanel),
...
def MenuEntryItem(entry):
    ...
    res.append(MultiContentEntryText(pos=(60, 10), size=(440, 40), font=0, text=entry[1]))
    return res
...
class Infopanel(Screen, InfoBarPiP):
    ...
    def __init__(self, session, services = None):
        ...
        self.Mlist = []
        self.Mlist.append(MenuEntryItem((InfoEntryComponent('CamdMenager'), _('Camd-Menager'), 'CamdMenager')))
    ...
    def ok(self):
        ...
        elif menu == 'CamdMenager':
            from Plugins.Extensions.CamdMenager.plugin import CamdMenager	
            self.session.open(CamdMenager)
